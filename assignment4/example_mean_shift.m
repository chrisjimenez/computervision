...
% Chris Jimenez
% Originally written by Rob Fergus, edited by Chris Jimenez

function [segmented_im] = example_mean_shift(input_im,window_size)
  
convergence_threshold = 1e-2;
cluster_quantization = 1; %% larger means fewer clusters
...
for i=1:nPixels
  ...
  while ~converged
    ...
    idx = find(compute_L2_dist(center, pixels) <= window_size);
    
    % now update center location
    new_center = mean(pixels(idx,:),1);
